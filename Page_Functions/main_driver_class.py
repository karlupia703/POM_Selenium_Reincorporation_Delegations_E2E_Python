...
class MainDriverClass:
    driver = None
    language = "en_US"  # Options: "Español", "en_US", "pt_BR", "it_IT"

    @classmethod
    def setup_language(cls):
        """Setup language for test execution"""
        print(f"Running tests in language: {cls.language}")
        os.environ["TEST_LANGUAGE"] = cls.language  # Store language in environment

    @classmethod
    def run(cls):
        """Main method to execute login tests"""
        print("Starting main method...")

        # Initialize login test
        login_test = TestLogin()  # ✅ Removed language argument
        login_test.setup_method()  # Setup driver
        cls.driver = login_test.driver

        login_test.test_login_user(cls.language)  # ✅ Pass language in method, not constructor
